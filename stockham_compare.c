...
void cpu_vadd(long N, long pN, long p, long a, long b, long g, long *x, long *x_copy, long *rou, long kk) {
...
int main(int argc, char** argv) {
    ...
    for (long n = 4; n <= 16; n++) {
        ...
        for (int iii = 0; iii < 10; iii++) {
            ...
            if (f == NULL || x == NULL || x_copy == NULL || cpu_x == NULL || cpu_x_copy == NULL) {
            ...
            for (long i = 0; i < n; i++) {
                ...
                // CPU でも同様の計算を実行
                cpu_vadd(N, pN, p, a, b, g, cpu_x, cpu_x_copy, rou, kk);
            ...
            // GPUの結果とCPUの結果を比較して検証
            if (!verify_results(x_copy, cpu_x_copy, N)) {
                printf("Results do not match for N = %ld\n", N);
            } else {
                printf("Results match for N = %ld\n", N);
            }
        }
